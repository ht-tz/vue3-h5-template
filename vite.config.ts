...
import externalGlobals from 'rollup-plugin-external-globals'; //外链需要安装的依赖
...
export default defineConfig(async ({ mode }: ConfigEnv): Promise<UserConfig> => {
    ...
    return {
        ...
        build: {
            ...
            sourcemap: true, // 生产环境古纳比sourceMap
            // 打包大小超出 400kb 提示警告
            ...
            rollupOptions: {
                ...
                external: ['moment', 'video.js', 'jspdf', 'xlsx', 'echart'], //第三方的文件不打包，外链引入文件
                plugins: [globals], // rollup 打包分析插件 生成矩形图
                experimentalLogSideEffects: true, //将每一个文件发现的第一个副作用打印到控制台上，
                treeshake: {
                    preset: 'recommended'
                    // propertyReadSideEffects: true
                },
                output: {
                    experimentalMinChunkSize: 20 * 1024, //默认值为1, 以一个字节为最小单位
                    manualChunks: (id: string) => {
                        // 极少数页面使用，所=以需要单独处理下，第三方库分类打包
